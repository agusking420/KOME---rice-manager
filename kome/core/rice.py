"""Rice data model — represents a single rice directory."""
...
class MappingEntry:
    ...
    target: str  # Absolute destination (~ expanded)
...
@dataclass
class Rice:
    """Represents a rice stored in the KOME rices directory."""
    ...
    def from_directory(cls, path: Path) -> "Rice":
        """Build a Rice from a directory. Raises ValueError if invalid."""
        path = path.resolve()
        if not path.is_dir():
        ...
        if not config_path.is_dir() and not has_mapping:
            raise ValueError(
                f"Rice '{path.name}' is missing '{RICE_CONFIG_SUBDIR}/' "
                f"and has no '{RICE_MAPPING_FILE}'."
            )
    ...
    def get_config_entries(self) -> list[Path]:
        """Return top-level entries inside the rice's .config/ directory."""
        if not self.config_path.is_dir():
            return []
        return sorted(self.config_path.iterdir())

    def get_missing_deps(self) -> list[str]:
        """Return dependencies not installed on the system."""
    ...
    def validate(self) -> list[str]:
        """Validate structure, return list of warnings (empty = valid)."""
        warnings: list[str] = []
...
def _parse_mapping_file(path: Path) -> list[MappingEntry]:
    """Parse mapping.json → list of MappingEntry."""
    try:
